...
export default class Task {
  ...
  */
  async dockerConfig(links) {
    ...
    if (this.runtime.ci) {
      // Consider loading temp credentials. Refer to Bug 1134342
      let testCreds = [
        `TASKCLUSTER_CLIENT_ID=${this.runtime.taskcluster.clientId}`,
        `TASKCLUSTER_ACCESS_TOKEN=${this.runtime.taskcluster.accessToken}`,
        `PULSE_USERNAME=${this.runtime.pulse.username}`,
        `PULSE_PASSWORD=${this.runtime.pulse.password}`
      ];

      procConfig.create.Env = procConfig.create.Env.concat(testCreds);
    }
